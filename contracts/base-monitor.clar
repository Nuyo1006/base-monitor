;; base-monitor
;; A smart contract for tracking and monitoring blockchain-based activities
;; This contract enables users to create comprehensive monitoring targets, track performance metrics,
;; and generate verifiable insights into on-chain events and interactions.

;; Error codes
(define-constant ERR-NOT-AUTHORIZED (err u100))
(define-constant ERR-TARGET-NOT-FOUND (err u101))
(define-constant ERR-INVALID-CATEGORY (err u102))
(define-constant ERR-INVALID-COMPLEXITY (err u103))
(define-constant ERR-EVENT-ALREADY-LOGGED (err u104))
(define-constant ERR-TARGET-INACTIVE (err u105))
(define-constant ERR-METRIC-NOT-FOUND (err u106))
(define-constant ERR-UNAUTHORIZED-ACCESS (err u107))
(define-constant ERR-COMPUTATION-FAILED (err u108))
(define-constant ERR-INSUFFICIENT-PERMISSIONS (err u109))
(define-constant ERR-DATA-VALIDATION-FAILED (err u110))

;; Constants
(define-constant CONTRACT-OWNER tx-sender)
(define-constant CATEGORY-BLOCKCHAIN u1)
(define-constant CATEGORY-SMART-CONTRACT u2)
(define-constant CATEGORY-TRANSACTION u3)
(define-constant CATEGORY-NETWORK u4)

(define-constant COMPLEXITY-LOW u1)
(define-constant COMPLEXITY-MEDIUM u2)
(define-constant COMPLEXITY-HIGH u3)
